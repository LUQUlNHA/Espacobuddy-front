...
export default function CadastroAlimentador() {
  ...
  const [estoqueKg, setEstoqueKg] = useState('');
  ...
  const salvarAlimentador = async () => {
    ...
    if (!estoqueKg.trim()) {
      Alert.alert('Erro', 'Informe o estoque de ração atual.');
      return;
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        style={styles.input}
        placeholder="Estoque de ração atual (kg)"
        value={estoqueKg}
        onChangeText={(text) => {
          // Permitir apenas números e ponto
          const numericText = text.replace(/[^0-9.]/g, '');
          setEstoqueKg(numericText);
        }}
